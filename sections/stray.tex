...
\gls{L2} Bananas & \gls{L2} support pads & \gls{LWS} installation & 0.73\% & 0.7\% \\
Rivet Holes & Holes in Top-end Baffle & Sealing of holes & 0.45\% & -- \\
Sharp Arcs & \gls{L2} edges \dag & \gls{TBD} & \gls{TBD} & 1\% $\Upsilon$ \\
Submarine & \gls{L1}/L2 ghosts \dag & \gls{LWS} installation (TBC) & 0.7\% $\Upsilon$ & 8.5\%* \\
...
Other features such as the \textit{L3 Horseshoe}, arise from edge diffraction at \gls{LSSTCam}'s tertiary lens (\gls{L3}) near the field-of-view edge ($\sim1\fdeg99$ and $2\fdeg23$).
Ray tracing simulations also revealed scattered light from the \gls{M2} optical baffle, whose residual reflectance in the longer wavelength bands ($i$, $z$, $y$) can reach up to 30\%.
