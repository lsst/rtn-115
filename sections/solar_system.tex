...
\texttt{DiaSource} observations and previously known objects.  Rubin also
searched for and discovered new solar system objects, but those activities
were carried out in Prompt Processing rather than as part of the DP2 data
release processing and will be described in a forthcoming series of papers.
This release instead delivers observations of known sources whose positions
were associated with objects in the MPC orbit catalog, as described in
Section~\ref{sec:ss-association}.  DP2 does not provide an orbit catalog
derived by Rubin from these observations.  It does provide a Rubin-derived
catalog of object-level photometry, including per-band absolute magnitudes,
in the \texttt{SSObject} table.

DP2 provides five solar system tables (Table~\ref{tab:ss-tables}).  Two are
derived from the Rubin observations: \texttt{SSSource} contains one row per
associated detection and links measured \texttt{DiaSource} photometry and
astrometry to a predicted object, while \texttt{SSObject} contains one row
per object and summarizes its observations, per-band photometric properties,
and selected orbit-derived quantities.  The remaining three tables are
snapshot copies of tables in the Minor Planet Center (MPC) PostgreSQL
database, supplied as reference catalogs for orbital information and object
cross-identification.  Their upstream definitions and field semantics are
...
The Rubin-derived
tables contain 8,149,633
\texttt{SSSource} rows for 299,819 distinct objects, with the same 299,819
objects represented in \texttt{SSObject}.  Every \texttt{diaSourceId} occurs
at most once.  These are known-object products, not a blind moving-object
search or an orbit catalog derived from DP2 alone.  The associated sample can
nevertheless include objects first discovered by Rubin: if Prompt Processing
reported such an object to the MPC and it entered the MPC catalog before the
DP2 processing cutoff, it was eligible for the same known-object association.
The Prompt Processing discovery campaign preceded the official DP2 data
release processing specifically so that newly reported objects entering the
MPC catalog before the cutoff could be treated as known objects during
association.  Those discoveries were made in Prompt Processing and will be
presented separately.  DP2 does not deliver a catalog of Rubin discoveries;
future Data Releases will deliver such catalogs.

\begin{deluxetable*}{lrrp{0.55\textwidth}}
...
\startdata
\texttt{SSSource} & 8,149,633 & 39 & One row per associated
...
\texttt{SSObject} & 299,819 & 80 & One row per recovered known object,
including sampling summaries, phase-curve fits, and selected quantities
derived from the input orbit. \\
\texttt{mpc\_orbits} & TBD\tablenotemark{a} & 53 & MPC snapshot of
...
\texttt{current\_identifications} & TBD\tablenotemark{a} & 11 & MPC
...
\texttt{numbered\_identifications} & TBD\tablenotemark{a} & 11 & MPC
...
We provide three auxiliary tables adopted from the Minor Planet Center.  The
\texttt{mpc\_orbits} table records the MPC orbit solution and associated fit
information for designated objects.  The
...
information when available.  These tables retain the upstream MPC semantics
and are not recomputed from DP2 observations.  The
\texttt{mpc\_orbits} table available at ingestion did not contain comets.
...
The association input was a 2025 March 13 snapshot of the MPC orbit database,
limited to objects whose observational arcs exceeded two days.  Associations
were made independently for each detector and visit using positions predicted
by Sorcha \citep[][]{2025AJ....170..100M} at the visit epoch from this orbit
catalog.
...
relative to the DP2 prediction.  This well-constrained sample is a validation
test rather than a characterization of all objects.  Separate validation
against predictions from orbits fitted using only Gaia astrometry found that
the persistent $\sim30$~mas Rubin-minus-MPC offset disappeared.  This result
indicates that the offset is primarily associated with the MPC-based orbit
predictions rather than the Rubin astrometric calibration.  Its scale is
nevertheless appreciable relative to the median reported astrometric
uncertainty of 56~mas for an associated \texttt{DiaSource}.
