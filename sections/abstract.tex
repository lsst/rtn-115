\begin{abstract}
We present an early version of Rubin Data Preview 2 (DP2), the second data preview from the National Science Foundation--Department of Energy Vera C. Rubin Observatory.
\gls{EDP2} comprises coadds, detection catalogs, and ancillary data products; eventually, the full DP2 will also include single-epoch images and difference images, but any analysis begun on \gls{EDP2} will require no modification.
...
The median per-visit PSF FWHM across the wide-area survey ranges from \zmedianpsffwhmwide in the $z$ band to \grmedianpsffwhmwide in $g$ and $r$ bands.
COSMOS, the deepest field, reaches estimated coadded 5$\sigma$ depths of $u$=\cosmosumaglim, $g$=\cosmosgmaglim, $r$=\cosmosrmaglim, $i$=\cosmosimaglim, $z$=\cosmoszmaglim, $y$=\cosmosymaglim.
DP2 contains \nobjects distinct astrophysical objects, of which \nssobjects solar system objects.
DP2 is approximately \edptwosize in size and is available to Vera C. Rubin Observatory data rights holders via the Rubin Science Platform, a cloud-based environment for the analysis of petascale astronomical data.
